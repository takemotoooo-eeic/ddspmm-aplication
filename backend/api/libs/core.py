...
def split_params(
    ...
    Args:
        ...
        f0_diff_mean: f0_diffの平均（denormalize用）
        f0_diff_std: f0_diffの標準偏差（denormalize用）
        f0_original_mean: 元のf0の平均
        f0_original_std: 元のf0の標準偏差
        ...
        loudness_original_mean: 元のloudnessの平均（denormalize用）
        loudness_original_std: 元のloudnessの標準偏差（denormalize用）  
    ...
    """
    # f0の処理
    if direct_optim is not None and "pitch" in direct_optim:
        f0 = pitches.reshape(-1, f0_score.shape[1])
        f0_diff = f0 - denormalize(f0_score, f0_original_mean, f0_original_std)
    else:
        f0_normalized = aggregated[:, :, 0]  # (B, L)
        f0_diff = denormalize(f0_normalized, f0_diff_mean, f0_diff_std)
        f0_score = denormalize(f0_score, f0_original_mean, f0_original_std)
        f0 = f0_diff + f0_score

    # z_featureをdenormalize
    if direct_optim is not None and "z_feature" in direct_optim:
        z_feature = z_features.reshape(-1, z_features.shape[2], 16)
    else:
        z_feature_normalized = aggregated[:, :, 2:]  # (B, L, 16)
        z_feature = denormalize(z_feature_normalized, z_feature_mean, z_feature_std)

    # loudnessの処理
    if direct_optim is not None and "loudness" in direct_optim:
        loudness_normalized = loudnesses.reshape(-1, loudness_score.shape[1])
        loudness = denormalize(loudness_normalized, loudness_original_mean, loudness_original_std)
        loudness_diff = loudness - loudness_score
    else:
        ...
        loudness_normalized = normalize(loudness, loudness_original_mean, loudness_original_std)
    return f0, f0_diff, loudness_normalized, loudness_diff, z_feature
